...
<body>
	...
	<article>
		...
		<section>
			<h2 align="center">SOAL NOMOR 8</h2>
			<?php
			...
			<form method="post">
				<?php
				//<input type="hidden" name="kunci" value="membaca kondisi benar dan salah">
				?>
				<?php
					if($bacasoal['soal8'] != null) {
				?>
				<label>
					8. Jelaskan fungsi dari simbol Decision pada flowchart....  <br>
					<textarea name="soal8" placeholder="Isikan jawaban anda..."><?php echo $bacasoal['soal8']; ?></textarea>
				</label>
				<?php						
					} else {
				?>
				<label>
					8. Jelaskan fungsi dari simbol Decision pada flowchart....  <br>
					<textarea name="soal8" placeholder="Isikan jawaban anda..."></textarea>
				</label>
				<?php
					} 
				?>
			...
			<?php
				...
				if(isset($_POST['btnsoal1'])) {
					if(!empty($_POST['soal8'])) {
						$sqlupdate=mysqli_query($koneksi,"UPDATE tbl_soal SET soal8='$_POST[soal8]' WHERE id_peserta='$datapeserta[id]'");
