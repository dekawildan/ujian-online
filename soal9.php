...
<body>
	...
	<article>
		...
		<section>
			<h2 align="center">SOAL NOMOR 9</h2>
			<?php
			...
			<form method="post">
				<?php
				//<input type="hidden" name="kunci" value="mudah dipahami audien, membuat konsep kerja lebih teratur, membantu kita membuat rencana atau kerangka cerita, membantu memetakan pikiran">
				?>
				<?php
					if($bacasoal['soal9'] != null) {
				?>
				<label>
					9. Jelaskan manfaat dari menggunakan metode peta minda....  <br>
					<textarea name="soal9" placeholder="Isikan jawaban anda..."><?php echo $bacasoal['soal9']; ?></textarea>
				</label>
				<?php						
					} else {
				?>
				<label>
					9. Jelaskan manfaat dari menggunakan metode peta minda....  <br>
					<textarea name="soal9" placeholder="Isikan jawaban anda..."></textarea>
				</label>
				<?php
					} 
				?>
			...
			<?php
				...
				if(isset($_POST['btnsoal1'])) {
					if(!empty($_POST['soal9'])) {
						$sqlupdate=mysqli_query($koneksi,"UPDATE tbl_soal SET soal9='$_POST[soal9]' WHERE id_peserta='$datapeserta[id]'");
						header('location:soal10.php');
					} else {
						echo '<h3 align="center">Maaf Jawaban belum diisi</h3>';
					}
