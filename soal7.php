...
<body>
	...
	<article>
		...
		<section>
			<h2 align="center">SOAL NOMOR 7</h2>
			<?php
			...
			<form method="post">
				<?php
				//<input type="hidden" name="kunci" value="meminjam buku di perpustakaan, berangkat sekolah, upacara bendera, belajar di kelas">
				?>
				<?php
					if($bacasoal['soal7'] != null) {
				?>
				<label>
					7. Sebutkan contoh algoritma di kehidupan sekolah minimal 3 contoh....  <br>
					<textarea name="soal7" placeholder="Isikan jawaban anda..."><?php echo $bacasoal['soal7']; ?></textarea>
				</label>
				<?php						
					} else {
				?>
				<label>
					7. Sebutkan contoh algoritma di kehidupan sekolah minimal 3 contoh....  <br>
					<textarea name="soal7" placeholder="Isikan jawaban anda..."></textarea>
				</label>
				<?php
					} 
				?>
			...
			<?php
				...
				if(isset($_POST['btnsoal1'])) {
					if(!empty($_POST['soal7'])) {
						$sqlupdate=mysqli_query($koneksi,"UPDATE tbl_soal SET soal7='$_POST[soal7]' WHERE id_peserta='$datapeserta[id]'");
